...
import { AnalyticsReconciliationSettlementsQuerySchema } from './schema';
import { startOfAppDay as startOfUtcDay, endOfAppDay as endOfUtcDay } from '@/utils/app-time';
import {
  buildSettlementAnalytics,
  loadSettlementDetail,
  resolvePeriodFromPreset,
  type SettlementAnalyticsPayload,
} from './build-settlement-analytics';
...
/** menus.wifi.analytics.reconciliation.settlements @route /wifi/analytics/reconciliation/settlements */
export class AnalyticsReconciliationSettlementsController {
  ...
  public listOrDetails = [
    asyncController(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
      ...
      const { error } = AnalyticsReconciliationSettlementsQuerySchema.validate(req.query, {
        abortEarly: false,
      });
      if (error) {
      ...
      if (settlementId) {
        const detail = await loadSettlementDetail(this.prisma, orgIdParam, settlementId);
        if (!detail) {
      ...
      const analytics = await buildSettlementAnalytics(
        this.prisma,
        orgIdParam,
        periodFrom,
        periodTo,
      ...
      const payload: SettlementAnalyticsPayload & {
        periodFrom: string;
        periodTo: string;
        preset: PeriodPreset | null;
  ...
  public createOrUpdate = [
    asyncController(async (_req, res) => {
      responseError(res, 405, {
        ...
        message: 'Settlement analytics are read-only.',
      });
    }),
  ];
