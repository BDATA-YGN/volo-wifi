...
import {
  buildVoucherRunAnalytics,
  resolvePeriodFromPreset,
  type VoucherRunAnalyticsPayload,
} from './build-voucher-run-analytics';
...
/** menus.wifi.analytics.voucher-runs @route /wifi/analytics/voucher-runs */
export class AnalyticsVoucherRunsController {
  ...
  public listOrDetails = [
    asyncController(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
      ...
      const { error } = AnalyticsVoucherRunsQuerySchema.validate(req.query, { abortEarly: false });
      if (error) {
      ...
      if (batchId) {
        const batch = await this.prisma.voucherBatch.findFirst({
          where: { id: batchId, orgId: orgIdParam, deletedAt: null },
          select: { id: true },
        });
        if (!batch) {
          return responseError(res, 400, {
            code: 'INVALID_BATCH',
            message: 'Voucher run not found in this organization.',
          });
        }
      }

      const { periodFrom, periodTo, preset } = resolvePeriod(req.query);
      const analytics = await buildVoucherRunAnalytics(
        this.prisma,
        orgIdParam,
        periodFrom,
        periodTo,
      ...
      const payload: VoucherRunAnalyticsPayload & {
        periodFrom: string;
        periodTo: string;
        preset: PeriodPreset | null;
  ...
  public createOrUpdate = [
    asyncController(async (_req, res) => {
      responseError(res, 405, {
        ...
        message: 'Voucher run analytics are read-only.',
      });
    }),
  ];
