...
import {
  buildSessionTrafficAnalytics,
  resolvePeriodFromPreset,
  type SessionTrafficPayload,
} from './build-session-traffic-analytics';
...
/** menus.wifi.analytics.session-traffic @route /wifi/analytics/session-traffic */
export class AnalyticsSessionTrafficController {
  ...
  public listOrDetails = [
    asyncController(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
      ...
      const { error } = AnalyticsSessionTrafficQuerySchema.validate(req.query, {
        abortEarly: false,
      });
      if (error) {
      ...
      const analytics = await buildSessionTrafficAnalytics(
        this.prisma,
        orgIdParam,
        periodFrom,
        periodTo,
      ...
      const payload: SessionTrafficPayload & {
        periodFrom: string;
        periodTo: string;
        preset: PeriodPreset | null;
  ...
  public createOrUpdate = [
    asyncController(async (_req, res) => {
      responseError(res, 405, {
        ...
        message: 'Session traffic analytics are read-only.',
      });
    }),
  ];
