...
const API_ROUTES = RECEIPTS;

const createReceipt = async (payload: any): Promise<CommonResponse> => {
  try {
...
const updateReceipt = async (id: string, payload: any): Promise<CommonResponse> => {
  try {
...
const deleteReceipt = async (id: string): Promise<CommonResponse> => {
  try {
...
const getReceipts = async (id?: string, paginationParams?: PaginationParams): Promise<CommonListResponse | CommonResponse> => {
  try {
...
const testPrintReceipt = async (payload: any): Promise<CommonResponse> => {
  try {
    const res: AxiosResponse<any> = await apiClient.post<any>(API_ROUTES.testPrint(), payload);
    return res.data;
  } catch (error) {
    throw handleApiError(error);
  }
...
export { createReceipt, updateReceipt, deleteReceipt, getReceipts, testPrintReceipt };
