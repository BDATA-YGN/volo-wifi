...
const emptyFormOptions: ApprovalsFormOptions = {
  memberships: [],
  stations: [],
  resellers: [],
};

export function useAnalyticsReconciliationApprovals() {
  ...
  const selectStatus = useCallback((value: WorkflowStatus | undefined) => {
    setStatus(value);
  }, []);
  ...
  const loadDetail = useCallback(
    async (settlementId: string) => {
      ...
      try {
        const res = await Query.loadApprovalDetail({ orgId, settlementId });
