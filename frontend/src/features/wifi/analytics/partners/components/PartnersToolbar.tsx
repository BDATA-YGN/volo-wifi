...
import type { PartnerAnalyticsTab, PartnerSiteOption, PartnerTierOption } from "../types";
import { PARTNER_ANALYTICS_TABS } from "../constant";

const { RangePicker } = DatePicker;

type Props = {
  tab: PartnerAnalyticsTab;
  ...
  loading?: boolean;
  onTabChange: (tab: PartnerAnalyticsTab) => void;
    ...
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-3">
          <Segmented
            value={tab}
            options={PARTNER_ANALYTICS_TABS.map((item) => ({
              value: item.key,
              label: item.label,
            }))}
            onChange={(value) => onTabChange(value as PartnerAnalyticsTab)}
          />
          {tab !== "stats" ? (
            <Checkbox
