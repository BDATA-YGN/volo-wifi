...
const PartnersTrendChart: React.FC<Props> = ({
  ...
  const { revenueLine, tokenLine, activeLine } = useMemo(() => {
    ...
    const actives = points.map((p) => p.activePartners);
      ...
      {points.length === 0 ? (
        ...
        <>
          <div className="mb-3 flex flex-wrap gap-4">
            ...
            {showActivePartners ? <LegendLine color={COLORS.active} label="Active partners" /> : null}
          </div>
          ...
          >
            {hoverPoint && hoverX != null ? (
              ...
              >
                ...
                {showActivePartners ? <div>Active partners: {hoverPoint.activePartners}</div> : null}
              </div>
            ) : null}
