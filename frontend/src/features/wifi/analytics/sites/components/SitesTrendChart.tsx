...
const SitesTrendChart: React.FC<Props> = ({
  ...
  const maxActive = useMemo(() => Math.max(...points.map((p) => p.activeSites), 1), [points]);
        ...
        <>
          ...
          <div className="flex items-end gap-px overflow-x-auto pb-6" style={{ minHeight: 160 }}>
            {points.map((point, index) => {
              ...
              const activeHeight = showActiveSites
                ? Math.max(4, (point.activeSites / maxActive) * 120)
                : 0;
                ...
                <Tooltip
                  ...
                  title={
                    <div style={{ fontSize: 12 }}>
                      ...
                      {showActiveSites ? <div>Active sites: {point.activeSites}</div> : null}
                    </div>
                  }
                >
                  <div
