...
import type { TenantDailyPoint } from "../types";
...
const TenantsTrendChart: React.FC<Props> = ({
  points,
  currency,
  loading,
  showActiveTenants = true,
}) => {
  ...
  const maxActive = useMemo(
    () => Math.max(...points.map((p) => p.activeTenants), 1),
    [points]
  );
        ...
        <>
          ...
          <div className="flex items-end gap-px overflow-x-auto pb-6" style={{ minHeight: 160 }}>
            {points.map((point, index) => {
              ...
              const activeHeight = showActiveTenants
                ? Math.max(4, (point.activeTenants / maxActive) * 120)
                : 0;
                ...
                <Tooltip
                  ...
                  title={
                    <div style={{ fontSize: 12 }}>
                      ...
                      {showActiveTenants ? (
                        <div>Active tenants: {point.activeTenants}</div>
                      ) : null}
                    </div>
                  }
                >
...
export default TenantsTrendChart;
