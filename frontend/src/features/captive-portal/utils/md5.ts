/**
 * Minimal MD5 for MikroTik Hotspot HTTP-CHAP (browser-safe, no dependency).
 */
export function md5Hex(input: string): string {
  ...
  function toWords(str: string): number[] {
    ...
    for (let i = 0; i < length; i += 1) {
      ...
      words[index] |= str.charCodeAt(i) << ((i % 4) * 8);
    }
    words[(length >> 2) >>> 0] |= 0x80 << ((length % 4) * 8);
  ...
  function process(words: number[]): string {
    let a = 0x67452301;
    let b = 0xefcdab89;
    let c = 0x98badcfe;
    let d = 0x10225476;
    ...
    const ff = (
      ...
      const res = (aa + ((bb & cc) | (~bb & dd)) + x + t) | 0;
      return (rotateLeft(res, s) + bb) | 0;
    };
    const gg = (
      aa: number,
      bb: number,
      ...
      const res = (aa + ((bb & dd) | (cc & ~dd)) + x + t) | 0;
      return (rotateLeft(res, s) + bb) | 0;
    };
    const hh = (
      aa: number,
