...
interface PartnerChangePasswordDrawerProps {
  open: boolean;
  onClose: () => void;
}

export default function PartnerChangePasswordDrawer({
  open,
  onClose,
}: PartnerChangePasswordDrawerProps) {
  ...
  const onFinish = async (values: ChangePasswordForm) => {
    setSubmitting(true);
    try {
      const result = await PartnerAuth.partnerChangePassword({
      ...
      message.success("Password updated. Other devices were signed out.");
      handleClose();
    } catch (err) {
      ...
      <MobileDrawerBody actor="partner">
        <div className={styles.drawerBody}>
          <div className={styles.drawerHint}>
            Enter your current password, then choose a strong new one. You stay signed in on this
            device; other sessions are signed out.
          </div>
